# TinyBase Records API Documentation

**Version:** 2.1
**Base URL:** `http://localhost:5000/api/v1`

The Records API is the core interface for interacting with data stored in TinyBase. It allows you to Perform CRUD (Create, Read, Update, Delete) operations, complex filtering, relational expansion, and high-performance search.
...
A generic record in TinyBase consists of a system ID and a flexible JSON payload.
...
2.  **Relations:** If the schema defines a `relation`, TinyBase automatically updates the internal graph table.
...
### Filtering
TinyBase supports JSON-based filtering using the `filter` query parameter.
...
TinyBase solves the "N+1" problem using the `expand` parameter. It fetches related records in a single HTTP request using optimized Recursive CTEs (Common Table Expressions).
...
TinyBase offers two types of search mechanisms.
