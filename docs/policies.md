# TinyBase Security Policies & Access Control

**Version:** 2.4 (Updated for Expression Engine)
**Context:** Policies define **who** can perform **what** action (`read`, `create`, `update`, `delete`) on a Collection.

TinyBase uses a robust **Expression Engine** allowing for complex logic, role-based access control (RBAC), and row-level security (RLS) by chaining conditions.
...
TinyBase 2.4 maintains backward compatibility with v1.0 shorthands.
