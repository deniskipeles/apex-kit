...
use tinybase_core::{Db, TinyBase, VectorProvider};
use tinybase_core::search::SearchManager;
use apex_vector::{CandleEmbedder, VectorIndex}; 
use std::time::Duration;
use crate::storage::DynamicStorage;
use tinybase_core::cache::CachedDb;
use std::path::Path;
use tracing::info;
...
pub struct TenantManager {
    ...
    vault: Arc<tinybase_core::security::Vault>,
}

impl TenantManager {
    pub fn new(shared_embedder: Option<Arc<CandleEmbedder>>, vault: Arc<tinybase_core::security::Vault>, capacity: u64) -> Self {
    ...
    async fn load_tenant(&self, tenant_id: &str) -> Result<Arc<dyn Db>, String> {
        ...
        // 2. Initialize Database (Uses TinyBase Core Factory)
        // This connects to existing files or creates empty ones if create_tenant called it
        let mut tinybase = TinyBase::init_filesystem(&base_path, tenant_vector_provider.clone())
            .await
        ...
        tinybase.set_search_manager(search_manager);

        // 4. Initialize Storage
        let upload_path = format!("{}/uploads", base_path);
        let public_url = "/api/v1/storage/file/".to_string(); 
        
        let db_arc: Arc<dyn Db> = Arc::new(CachedDb::new(Arc::new(tinybase.clone())));
