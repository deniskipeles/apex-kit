...
use tinybase_core::realtime::EventScope;
    ...
    match SandboxManager::get_sandbox(session_id).await {
        Ok(sandbox_db) => {
            ...
            let storage: Arc<dyn StorageBackend> = Arc::new(
                tinybase_core::storage::LocalStorage::new(&storage_path, &public_url).await
            );
...
async fn tenant_resolver_middleware(
    ...
    match state.tenant_manager.get_tenant(tenant_id.clone()).await {
        Ok(tenant_db) => {
            ...
            let storage: Arc<dyn StorageBackend> = Arc::new(
                tinybase_core::storage::LocalStorage::new(&storage_path, "/api/v1/storage/file/").await
            );
            req.extensions_mut().insert(storage);
...
<html>
  <head>
    <title>TinyBase API Reference (Tenant)</title>
    <meta charset="utf-8" />
...
<html>
  <head>
    <title>TinyBase API Reference (Sandbox)</title>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <style>
      body {{ margin: 0; }}
...
async fn create_record(
    ...
    Json(p): Json<tinybase_core::models::Record>
...
#[utoipa::path(get, path = "/api/v1/collections/{id}/instant-search", params(SearchQuery), responses((status = 200, body = Vec<tinybase_core::models::InstantResult>)))]
pub async fn instant_search_handler(auth: Option<Extension<Claims>>, DatabaseConnection(db): DatabaseConnection, Path(path): Path<IdPath>, Query(params): Query<SearchQuery>) -> Result<Json<Vec<tinybase_core::models::InstantResult>>, AppError> {
    let claims = auth.map(|Extension(c)| c);
...
async fn trigger_hooks(
    ...
    collection: &tinybase_core::Collection, 
