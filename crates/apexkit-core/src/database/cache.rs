...
use crate::database::traits::{
    AiActionStore, AiSessionStore, ApiKeyStore, AuditStore, AuthTokenStore, CollectionStore,
    ConfigStore, ConnectionStore, DashboardStore, FileStore, OAuthStore, PluginStore,
    QueryEngineStore, RecordStore, RelationStore, SandboxStore, ScriptStore, SearchStore,
    TemplateStore, TenantStore, UserStore, VectorStore,
};
use crate::models;
use crate::models::ApiKey;
use crate::models::ListResult;
use crate::models::ai::{AiSession, Plugin};
use crate::models::{Collection, Record, ai as ai_models, schema::CollectionSchema};
...
#[async_trait]
impl CollectionStore for CachedDb {
...
impl RecordStore for CachedDb {
    ...
    async fn import_record(
        ...
        let res = self
            .inner
            .import_record(collection_id, record_id, data)
            .await;
        self.record_cache
            .invalidate(&(collection_id, record_id))
            .await;
        res
    }
    ...
    async fn get_record(
        ...
        if expand.as_ref().is_some_and(|ex| !ex.trim().is_empty()) {
            return self
                .inner
                .get_record(collection_id, record_id, expand)
                .await;
        }
        ...
        let result = self
            .inner
            .get_record(collection_id, record_id, None)
            .await?;
    ...
    async fn update_record(
        ...
        let res = self
            .inner
            .update_record(collection_id, record_id, data)
            .await?;
        self.record_cache
            .invalidate(&(collection_id, record_id))
            .await;
...
impl SearchStore for CachedDb {
    async fn instant_search(
        ...
        query: &str,
        limit: usize,
    ) -> Result<Vec<models::InstantResult>, Box<dyn std::error::Error + Send + Sync>> {
...
impl UserStore for CachedDb {
    async fn create_user(
        ...
        self.inner
            .create_user(email, password_hash, role, metadata)
            .await
    }
    ...
    async fn import_user(
        ...
        self.inner
            .import_user(id, email, password_hash, role, metadata)
            .await
    }
    ...
    async fn update_user(
        ...
        self.inner
            .update_user(id, email, role, metadata, password)
            .await
    }
}

#[async_trait]
impl OAuthStore for CachedDb {
...
#[async_trait]
impl AuthTokenStore for CachedDb {
    async fn create_auth_token(
        ...
        self.inner
            .create_auth_token(user_id, token_type, token)
            .await
    }
...
#[async_trait]
impl SandboxStore for CachedDb {
...
impl FileStore for CachedDb {
    async fn create_file_metadata(
        ...
        self.inner
            .create_file_metadata(filename, original_name, mime_type, size, user_id)
            .await
    }
...
impl AuditStore for CachedDb {
    async fn log_audit_event(
        ...
        self.inner
            .log_audit_event(level, message, source, meta)
            .await
    }
...
#[async_trait]
impl AiActionStore for CachedDb {
...
#[async_trait]
impl AiSessionStore for CachedDb {
...
#[async_trait]
impl RelationStore for CachedDb {
...
#[async_trait]
impl ScriptStore for CachedDb {
    async fn list_scripts(
        &self,
    ) -> Result<Vec<crate::models::script::Script>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_script_by_name(
        &self,
        name: &str,
    ) -> Result<Option<crate::models::script::Script>, Box<dyn std::error::Error + Send + Sync>>
    {
...
#[async_trait]
impl TemplateStore for CachedDb {
    async fn list_templates(
        &self,
    ) -> Result<Vec<models::Template>, Box<dyn std::error::Error + Send + Sync>> {
    ...
    async fn get_template_by_slug(
        &self,
        slug: &str,
    ) -> Result<Option<models::Template>, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(tmpl) = &result {
            self.template_cache
                .insert(slug.to_string(), Arc::new(tmpl.clone()))
                .await;
        }
        Ok(result)
    }
...
#[async_trait]
impl DashboardStore for CachedDb {
    async fn get_dashboard_stats(
        &self,
    ) -> Result<crate::models::DashboardData, Box<dyn std::error::Error + Send + Sync>> {
        self.inner.get_dashboard_stats().await
    }
}

#[async_trait]
impl VectorStore for CachedDb {
    async fn save_vector(
        ...
        self.inner
            .save_vector(collection_id, record_id, field_name, vector, model)
            .await
    }
    ...
    async fn has_vector(
        ...
        self.inner
            .has_vector(collection_id, record_id, field_name, model)
            .await
    }
    ...
    async fn get_record_vectors(
        ...
        self.inner
            .get_record_vectors(collection_id, record_id)
            .await
    }
    ...
    async fn search_vector(
        ...
        vector: Vec<f32>,
        limit: usize,
    ) -> std::result::Result<Vec<(Record, f32)>, Box<dyn StdError + Send + Sync>> {
        self.inner
...
#[async_trait]
impl QueryEngineStore for CachedDb {
    async fn query_engine(
        &self,
        query: crate::query::ApexQuery,
...
#[async_trait]
impl ConnectionStore for CachedDb {
