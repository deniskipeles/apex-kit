...
async fn test_nested_get_subquery_policies() {
    ...
    db.create_collection("bids", &Some(bids_schema), None)
        .await
        .unwrap();
    ...
    db.create_record(2, &json!({"tutorId": 1, "assignmentId": 999}))
        .await
        .unwrap();
    ...
    assert!(
        is_allowed,
        "Tutor who placed a bid on assignment 999 should be allowed"
    );

    // Fails for unbidded assignments (e.g., ID 888)
    let is_blocked = check_access(
        nested_policy,
        Some(&tutor_claims),
        Some(&json!({"id": 888})),
        None,
    ...
    assert!(
        !is_blocked,
        "Tutor who has not bidded on 888 should be blocked"
    );
...
async fn test_policy_sql_compilation() {
    ...
    let compiled_sql = compile_to_sql(string_rule, Some(&tutor_claims), None, None)
        .await
        .unwrap();
    assert_eq!(
        compiled_sql,
        "json_extract(records.data, '$.studentId') = '50'"
    );

    // Test compilation of nested logical Operators
    let logical_rule = "public && auth";
    let compiled_logical = compile_to_sql(logical_rule, Some(&tutor_claims), None, None)
        .await
        .unwrap();
    ...
    let compiled_nin = compile_to_sql(json_nin_rule, None, None, None)
        .await
        .unwrap();
...
async fn test_log_directive_policies() {
    ...
    assert!(
        is_allowed,
        "@log directive should not interfere with memory evaluation"
    );
    ...
    let (logs, total) = db
        .list_paginated_logs(
            ...
            10,
            None,
            Some("policy_debugger".to_string()),
            None,
        )
        .await
        .unwrap();
    assert!(
        total > 0,
        "Policy log should be stored inside _system_logs table"
    );
    assert!(
    ...
    let compiled = compile_to_sql(policy_with_log_sql, None, None, Some(db.clone()))
        .await
        .unwrap();
...
async fn test_request_and_record_spec_resolution() {
    ...
    let is_allowed = check_access(
        ...
        Some(&student_claims),
        None,
        Some(&incoming_request_data),
        Some(db.clone()),
    )
    .await;
    assert!(
        is_allowed,
        "The incoming request payload field senderId (1) matches their profile ID (1) and should pass"
    );
