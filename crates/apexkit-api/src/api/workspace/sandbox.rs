use crate::{AppError, AppState, workspaces_manager::sandbox::CloneStrategy};
use apexkit_core::{
    auth::Claims, models::SandboxMetadata, models::ai::Plugin, realtime::EventScope,
};
use axum::{
    Extension, Json,
    extract::{Path, State},
...
pub async fn create_sandbox_handler(
    ...
    let tenant_id = crate::utils::get_tenant_id_from_scope(Some(&event_scope));
    ...
    tokio::spawn(async move {
        if let Ok(sandbox_db) = state_clone.sandbox_manager.get_sandbox(&sid).await {
            let session = apexkit_core::models::ai::AiSession {
            ...
            if let Some(prompt) = prompt_opt {
                ...
                let _ = crate::api::ai::architect::process_ai_chat(
                    "default",
                    sandbox_db,
                    state_clone,
                    prompt,
...
pub async fn publish_sandbox_handler(
    ...
    let manifest_templates = templates
        ...
        .map(|t| {
            ...
            if let Some(sid) = t.script_id
                && let Some(s) = scripts.iter().find(|sc| sc.id == sid)
            {
    ...
    crate::api::ai::architect::deploy_manifest(parent_db.clone(), &manifest).await?;
