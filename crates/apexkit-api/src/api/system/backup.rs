use crate::system::dto::BackupConfigDto;
use crate::{AppError, AppState, DatabaseConnection}; // [FIX] Use DatabaseConnection extractor
...
pub async fn restore_handler(
    ...
    crate::system::backup::restore_backup(
        &file_path,
        false,
        db,
        state.vault.clone(),
    ...
    if matches!(event_scope, EventScope::Root) {
        ...
        Ok(Json(
            serde_json::json!({ "message": "Restoration successful. Server restarting..." }),
        ))
    } else {
        Ok(Json(
            serde_json::json!({ "message": "Restoration successful. Tenant data updated." }),
        ))
    }
}
...
pub async fn trigger_backup_handler(
    ...
    if !matches!(event_scope, EventScope::Root) && config.destination == "local" {
        ...
        let allow_val = state
            .db
            .get_config("ALLOW_NON_ROOT_BACKUP")
            .await
            .map_err(|e| AppError::UnknownError(e.to_string()))?;

        let allowed = allow_val
            .and_then(|v| v.as_str().map(|s| s == "true"))
            .unwrap_or(false);

        if !allowed {
            return Err(AppError::Forbidden(
                "Local backups disabled for tenants by Root Policy. Configure S3.".into(),
            ));
        }
    }
    ...
    tokio::spawn(async move {
        if let Err(e) = crate::system::backup::perform_backup(
            db_clone,
    ...
    Ok(Json(
        serde_json::json!({ "message": "Backup job started in background." }),
    ))
}
...
pub async fn list_backups_handler(
    ...
    if config.destination == "s3" {
        let storage_settings = db
            .get_config("storage")
            .await
            .map_err(|e| AppError::UnknownError(e.to_string()))?;
        if let Some(val) = storage_settings {
            let s_conf: crate::system::dto::StorageConfigDto =
                serde_json::from_value(val).unwrap_or_default();
            if s_conf.s3.enabled {
                let secret = if let Some(enc_str) = s_conf.s3.secret_key {
                    let enc: EncryptedValue = serde_json::from_str(&enc_str)
                        .map_err(|_| AppError::UnknownError("Bad secret format".into()))?;
                    state
                        .vault
                        .decrypt(&enc)
        ...
        if path.exists() {
            let entries =
                std::fs::read_dir(path).map_err(|e| AppError::UnknownError(e.to_string()))?;
...
pub async fn restore_from_file_handler(
    ...
    crate::system::backup::restore_backup(&path, false, db, state.vault.clone(), event_scope)
        .await
        .map_err(AppError::UnknownError)?;

    // Only restart if root
    if path.contains("storage/backups/") {
        // Rudimentary check for root
        tokio::spawn(async {
            tokio::time::sleep(std::time::Duration::from_secs(1)).await;
            std::process::exit(0);
        });
    ...
    Ok(Json(
        serde_json::json!({ "message": "Restoration successful." }),
    ))
}
