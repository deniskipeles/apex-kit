...
use crate::schema::{CollectionPolicies, CollectionSchema};
use crate::security::Vault;
use crate::storage::StorageBackend;
use async_trait::async_trait;
use chrono::Utc;
use models::{ChangesetEvent, Collection, ListResult, Record};
...
pub trait ScriptContext: Send + Sync {
    ...
    fn get_shared_script(
        ...
        Box<dyn std::future::Future<Output = Option<crate::script_models::Script>> + Send>,
    >;
...
pub trait Db: Send + Sync {
    ...
    async fn list_scripts(
        &self,
    ) -> std::result::Result<Vec<script_models::Script>, Box<dyn std::error::Error + Send + Sync>>;
    async fn create_script(
        &self,
        req: script_models::CreateScriptReq,
    ) -> std::result::Result<i64, Box<dyn std::error::Error + Send + Sync>>;
    ...
    async fn get_script_by_name(
        &self,
        name: &str,
    ) -> std::result::Result<Option<script_models::Script>, Box<dyn std::error::Error + Send + Sync>>;
...
impl ApexKit {
    ...
    async fn get_core_read<'a>(&'a self) -> tokio::sync::MutexGuard<'a, Connection> {
        self.hot_conn_core.lock().await
    }
    async fn get_data_read<'a>(&'a self) -> tokio::sync::MutexGuard<'a, Connection> {
        self.hot_conn_data.lock().await
    }
    async fn get_log_read<'a>(&'a self) -> tokio::sync::MutexGuard<'a, Connection> {
        self.hot_conn_log.lock().await
    }
    async fn get_sys_read<'a>(&'a self) -> tokio::sync::MutexGuard<'a, Connection> {
        self.hot_conn_sys.lock().await
    }
    async fn get_vector_read<'a>(&'a self) -> tokio::sync::MutexGuard<'a, Connection> {
...
impl Db for ApexKit {
    ...
    async fn list_records(
        ...
        let (mut records, total) = {
            ...
            let builder = query::SqlBuilder::new(
                collection_id,
                &current_col_name,
                options.clone(),
                &schema_map,
    ...
    async fn create_user(
        ...
        let id = self
            .core_batcher
            .insert(
                "INSERT INTO users (email, password_hash, role, metadata) VALUES (?1, ?2, ?3, ?4)"
                    .into(),
                vec![
                    e.into_val(),
                    p.into_val(),
    ...
    async fn import_user(
        ...
        let meta = metadata.unwrap_or(json!({}));
        let meta_str = serde_json::to_string(&meta)?;
        self.core_batcher.execute("INSERT INTO users (id, email, password_hash, role, metadata) VALUES (?1, ?2, ?3, ?4, ?5)".into(), vec![id.into_val(), e.into_val(), p.into_val(), r.into_val(), meta_str.into_val()]).await.map_err(|e| Box::new(std::io::Error::other(e)))?;
        Ok(())
    }
    ...
    async fn get_user_by_email(
        ...
        let mut stmt = conn.prepare(
            "SELECT id, email, password_hash, role, metadata FROM users WHERE email = ?1",
        )?;
    ...
    async fn update_user(
        ...
        if sets.is_empty() {
            let conn = self.get_core_read().await;
            let mut stmt = conn.prepare(
                "SELECT id, email, password_hash, role, metadata FROM users WHERE id = ?1",
            )?;
    ...
    async fn log_audit_event(
        ...
        self.log_batcher
            .execute(
                "INSERT INTO _audit_logs (level, message, source, meta) VALUES (?1, ?2, ?3, ?4)"
                    .into(),
                vec![
                    level.into_val(),
                    message.into_val(),
    ...
    ) -> std::result::Result<Vec<ai_models::AiAction>, Box<dyn std::error::Error + Send + Sync>>
    {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, slug, name, model, system_prompt, template, config FROM _ai_actions",
        )?;
        let mut rows = stmt.query([])?;
        let mut res = Vec::new();
        while let Some(row) = rows.next()? {
    ...
    async fn query_engine(
        ...
        let (sql, params, pipeline) =
            crate::query_engine::QueryBuilder::build(&query, self).await?;
