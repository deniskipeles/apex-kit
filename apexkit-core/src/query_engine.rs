...
impl QueryBuilder {
    pub async fn build(
        query: &ApexQuery,
        db: &impl Db,
    ) -> Result<(String, Vec<rusqlite::types::Value>, Vec<PipelineStep>), String> {
        ...
        let (table_name, where_prefix) = if is_system {
            ...
            let col = cols
                .iter()
                .find(|c| c.name == query.from)
                .ok_or_else(|| format!("Collection '{}' not found", query.from))?;
            (
                "records".to_string(),
                format!("WHERE collection_id = {}", col.id),
            )
        };
        ...
        if !is_system && let Some(rls) = &query.rls_sql {
        ...
        if !query.group_by.is_empty() {
            for g in &query.group_by {
                group_clauses.push(if is_system {
                    g.clone()
                } else {
                    Self::resolve_column(g)
                });
            }
        }
        let group_sql = if group_clauses.is_empty() {
            "".to_string()
        } else {
            format!("GROUP BY {}", group_clauses.join(", "))
        };

        // 5. Build ORDER BY
        let order_sql = if let Some(sort_str) = &query.sort {
            let parts: Vec<String> = sort_str
                .split(',')
                .map(|s| {
                    let s = s.trim();
                    let desc = s.starts_with('-');
                    let clean = s.trim_start_matches('-');
                    let col = if is_system {
                        clean.to_string()
                    } else {
                        Self::resolve_column(clean)
                    };
