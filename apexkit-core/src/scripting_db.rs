use std::sync::Arc;
use boa_engine::{
    Context, JsString, NativeFunction, JsValue, JsArgs,
...
pub async fn resolve_db(ctx_str: Option<String>, app_ctx: Arc<dyn ScriptContext>) -> Result<Arc<dyn Db>, String> {
...
pub fn create_db_object(ctx: &mut Context, mode: DbMode) -> Result<boa_engine::object::JsObject, String> {
    ...
    let list_records = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            ...
            let opts_val = args.get_or_undefined(offset + 1).to_json(ctx).unwrap().unwrap_or(serde_json::Value::Null);
    ...
    let get_record = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            let (ctx_id, offset) = get_args(args, ctx, &m);
            let col = args.get_or_undefined(offset).to_string(ctx)?.to_std_string_escaped();
            let id = args.get_or_undefined(offset + 1).to_number(ctx).unwrap_or(0.0) as i64;
            let expand = args.get(offset + 2).and_then(|v| v.as_string()).map(|s| s.to_std_string_escaped());
    ...
    let create_record = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            let (ctx_id, offset) = get_args(args, ctx, &m);
            let col = args.get_or_undefined(offset).to_string(ctx)?.to_std_string_escaped();
            let data = args.get_or_undefined(offset + 1).to_json(ctx).unwrap().unwrap_or(serde_json::Value::Null);
    ...
    let update_record = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            ...
            let col = args.get_or_undefined(offset).to_string(ctx)?.to_std_string_escaped();
            let id = args.get_or_undefined(offset + 1).to_number(ctx).unwrap_or(0.0) as i64;
            let data = args.get_or_undefined(offset + 2).to_json(ctx).unwrap().unwrap_or(serde_json::Value::Null);
    ...
    let search_vector = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            let (ctx_id, offset) = get_args(args, ctx, &m);
            let col = args.get_or_undefined(offset).to_string(ctx)?.to_std_string_escaped();
            let field = args.get_or_undefined(offset + 1).to_string(ctx)?.to_std_string_escaped();
            let vec_val = args.get_or_undefined(offset + 2).to_json(ctx).unwrap().unwrap_or(serde_json::Value::Null);
            let limit = args.get_or_undefined(offset + 3).to_number(ctx).unwrap_or(10.0) as usize;
    ...
    let files_list = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            ...
            let offset_val = args.get_or_undefined(offset + 1).to_number(ctx).unwrap_or(0.0) as i64;

            let res = ACTIVE_CONTEXT.with(|c| {
                if let Some((app, handle, _, _, _)) = &*c.borrow() {
                    handle.block_on(async {
                        let db = resolve_db(ctx_id, app.clone()).await?;
                        let files = db.list_files(limit, offset_val).await.map_err(|e| e.to_string())?;
    ...
    let users_get = {
        ...
        NativeFunction::from_copy_closure(move |_, args, ctx| {
            let (ctx_id, offset) = get_args(args, ctx, &m);
            let email = args.get_or_undefined(offset).to_string(ctx)?.to_std_string_escaped();
    ...
    let users_create = {
         ...
         NativeFunction::from_copy_closure(move |_, args, ctx| {
             let (ctx_id, offset) = get_args(args, ctx, &m);
             let email = args.get_or_undefined(offset).to_string(ctx)?.to_std_string_escaped();
             let pass = args.get_or_undefined(offset + 1).to_string(ctx)?.to_std_string_escaped();
             let role = args.get_or_undefined(offset + 2).to_string(ctx)?.to_std_string_escaped();
