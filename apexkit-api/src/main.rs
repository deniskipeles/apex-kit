use apexkit_api::{
    AppState, GlobalJobContext, app_router,
    cli::{self, Cli},
};
use apexkit_core::{
    Db, VectorProvider, a_new_database_connection,
...
use apex_vector::{CandleEmbedder, EmbeddingModelConfig, VectorEngine};

use apexkit_api::replication::{
...
async fn main() {
    ...
    tracing::info!(
        "Active Vector Model: {} (Context: {}, Overlap: {})",
        active_model_name,
    ...
    let (sqlite_event_tx, _sqlite_event_rx) =
        tokio::sync::broadcast::channel::<ChangesetEvent>(1000);
    ...
    let storage: Arc<dyn StorageBackend> = Arc::new(apexkit_api::storage::DynamicStorage::new(
        cached_db.clone(),
        vault.clone(),
        None,
    ...
    let empty_schema = {
        ...
        builder
            .register(query)
            .finish()
            .expect("Failed to build empty schema")
    };
    ...
    if let Ok(master_url) = std::env::var("APEX_MASTER_URL") {
        if !master_url.is_empty() {
            ...
            tokio::spawn(async move {
                apexkit_api::replication::start_event_streamer(
                    master_url,
            ...
            tokio::spawn(async move {
                loop {
                    if let Ok(event) = rx.recv().await {
                        let db_path = apexkit_api::replication::get_db_path_from_scope(
                            &event.scope,
                            &event.db_name,
                        );
                        ...
                        tokio::task::spawn_blocking(move || {
                            if let Ok(conn) = rusqlite::Connection::open(&db_path) {
                                ...
                                if let Err(e) = conn.apply_strm(
                                    ...
                                    tracing::error!(
                                        "❌ [Replica] Failed to apply changeset to {}: {}",
                                        db_path,
                                        e
                                    );
                                }
    ...
    let gql_schema = apexkit_api::graphql::build_schema(state.clone(), relation_loader)
        .await
    ...
    if is_replica {
        ...
        if let Err(e) = axum::serve(
            listener,
            multiplexed_app.into_make_service_with_connect_info::<std::net::SocketAddr>(),
        )
        .await
        {
            tracing::error!("Server error: {}", e);
