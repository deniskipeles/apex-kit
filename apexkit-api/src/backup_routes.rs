...
pub async fn trigger_backup_handler(
    ...
    tokio::spawn(async move {
        if let Err(e) =
            crate::backup::perform_backup(db_clone, state_clone.vault.clone(), config, event_scope)
                .await
        {
...
pub async fn list_backups_handler(
    ...
    if config.destination == "s3" {
        ...
        if let Some(val) = storage_settings {
            let s_conf: crate::settings::StorageConfigDto =
                serde_json::from_value(val).unwrap_or_default();
...
pub async fn restore_from_file_handler(
    ...
    crate::backup::restore_backup(&path, false, db, state.vault.clone(), event_scope)
        .await
        .map_err(|e| AppError::UnknownError(e))?;
