...
async fn resolve_db_from_scope(
    state: &AppState,
    scope: &EventScope,
...
impl apexkit_core::ScriptContext for ScopedScriptContext {
    ...
    fn get_shared_script(
        ...
        Box<dyn std::future::Future<Output = Option<apexkit_core::script_models::Script>> + Send>,
    > {
...
pub async fn serve_styles(
    ...
    let css = css_compiler::compile_styles(db.clone())
        .await
        .map_err(AppError::UnknownError)?;
...
fn make_api_router() -> Router<AppState> {
    ...
    Router::new()
        ...
        .route(
            "/auth/verify/resend",
            post(auth_advanced::resend_verification),
        )
        .route(
            "/auth/request-password-reset",
            post(auth_advanced::request_password_reset),
        )
        .route(
            "/auth/confirm-password-reset",
            post(auth_advanced::confirm_password_reset),
        )
        .route(
            "/collections",
            post(collections_and_records_routes::create_collection)
                .get(collections_and_records_routes::list_collections),
        )
        .route(
            "/collections/{id}",
            get(collections_and_records_routes::get_collection)
                .patch(collections_and_records_routes::update_collection)
                .put(collections_and_records_routes::update_collection)
                .delete(collections_and_records_routes::delete_collection),
        )
        .route(
            "/collections/{id}/records",
            post(collections_and_records_routes::create_record)
                .get(collections_and_records_routes::list_records),
        )
        // Advanced Query Endpoint
        .route(
            "/collections/{id}/query",
            post(collections_and_records_routes::query_records_handler),
        )
        .route(
            "/collections/{id}/records/{record_id}",
            get(collections_and_records_routes::get_record)
                .patch(collections_and_records_routes::update_record)
                .put(collections_and_records_routes::update_record)
                .delete(collections_and_records_routes::delete_record),
        )
        .route(
            "/collections/{id}/search",
            get(collections_and_records_routes::search_records),
        )
        .route(
            "/collections/{id}/instant-search",
            get(collections_and_records_routes::instant_search_handler),
        )
        .route(
        ...
        .route(
            ...
            post(collections_and_records_routes::create_relation)
                .delete(collections_and_records_routes::delete_relation),
        )
        ...
        .route(
            "/admin/keys",
            get(key_routes::list_keys).post(key_routes::create_key),
        )
        .route(
            "/admin/keys/{id}",
            axum::routing::delete(key_routes::delete_key)
                .patch(key_routes::update_key)
                .put(key_routes::update_key),
        )
        ...
        .route(
            "/admin/users/{id}",
            axum::routing::delete(auth_advanced::delete_user_handler)
                .patch(auth_advanced::update_user_handler)
                .put(auth_advanced::update_user_handler),
        )
        .route("/admin/logs", get(list_audit_logs))
        .route("/admin/dashboard", get(get_dashboard_stats_handler))
        .route(
            "/admin/import-data",
            post(import_data_routes::import_data_handler),
        )
        .route(
            "/admin/export-data/{id}",
            get(export_data_routes::export_data_handler),
        )
        .route(
            "/admin/import-schema",
            post(import_data_routes::import_schema_handler),
        )
        .route(
            "/admin/export-schema",
            get(export_data_routes::export_schema_handler),
        )
        .route(
            "/admin/export-scripts",
            get(export_data_routes::export_scripts_handler),
        )
        .route(
            "/admin/export-templates",
            get(export_data_routes::export_templates_handler),
        )
        .route(
            "/admin/export-ai-actions",
            get(export_data_routes::export_ai_actions_handler),
        )
        .route(
            "/admin/import-scripts",
            post(import_data_routes::import_scripts_handler),
        )
        .route(
            "/admin/import-templates",
            post(import_data_routes::import_templates_handler),
        )
        .route(
            "/admin/import-ai-actions",
            post(import_data_routes::import_ai_actions_handler),
        )
        .route(
            "/admin/collections/{id}/reindex",
            post(reindex_collection_handler),
        )
        .route(
        ...
        .route(
            "/admin/site/files",
            get(site_routes::list_site_files_handler).delete(site_routes::delete_site_file_handler),
        )
...
pub fn app_router(state: AppState) -> Router {
    ...
    let renderer_routes = Router::new().route(
        "/render/{*slug}",
        get(renderer::render_view).post(renderer::render_view),
    );
    ...
    let sandbox_router = Router::new()
        ...
        .route(
            "/render/{*slug}",
            get(renderer::render_sandbox_view).post(renderer::render_sandbox_view),
        )
        .route(
            "/graphql",
    ...
    let tenant_path_router = Router::new()
        ...
        .route(
            "/render/{*slug}",
            get(renderer::render_tenant_view).post(renderer::render_tenant_view),
        )
        .route(
            "/graphql",
