...
export const ConfirmDialog = ({ isOpen, title, description, onConfirm, onCancel, variant = 'destructive', confirmText = 'Confirm', isLoading }: ConfirmDialogProps) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 isolate">
       ...
       <div className="relative w-full max-w-md bg-background rounded-lg border border-border shadow-xl p-6 animate-in zoom-in-95 duration-200">
          <div className="flex gap-4">
             <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 ${variant === 'destructive' ? 'bg-destructive/10 text-destructive' : 'bg-primary/10 text-primary'}`}>
