...
const ImageInsertionModal: React.FC<ImageInsertionModalProps> = ({ onClose, onInsert }) => {
  ...
  const [aspectRatio, setAspectRatio] = useState<'aspect-auto' | 'aspect-square' | 'aspect-video'>(
    'aspect-auto'
  );
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4 transition-opacity duration-300">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl h-[90vh] flex flex-col animate-fade-in-up overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                ...
                <div className="animate-fade-in-up">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="e.g., Make it a painting in Van Gogh style..."
                      className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm dark:bg-gray-800 dark:text-white mb-2 focus:ring-2 focus:ring-blue-500"
                      rows={3}
                    />
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-2 px-4 bg-purple-600 hover:bg-purple-700 text-white rounded-md text-sm disabled:opacity-50 flex items-center justify-center gap-2 transition-colors"
                      >
                        {isEditing ? (
                          'Processing...'
                        ) : (
                          <>
                            <MagicWandIcon className="w-4 h-4" /> Generate Edit
                          </>
                        )}
                      </button>
                      <button
                        onClick={() => setEditingIndex(null)}
                        className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm hover:bg-gray-200 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300"
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 mb-2">
                      <input
                        ...
                        placeholder="https://..."
                        className="flex-1 p-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-blue-500"
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2">
                      <button
                        ...
                        className="py-2 px-3 border border-gray-300 dark:border-gray-600 rounded-md text-xs text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-800 flex items-center justify-center gap-1"
                      >
                    ...
                    <div className="p-3 bg-purple-50 dark:bg-gray-800 border border-purple-100 dark:border-purple-900 rounded-md">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 p-1.5 border border-purple-200 dark:border-gray-600 rounded text-xs dark:bg-gray-900 dark:text-white focus:ring-1 focus:ring-purple-500"
                          onKeyDown={(e) =>
                            e.key === 'Enter' && !e.shiftKey && handleGenerateImage()
                          }
                        />
                        <button
                          onClick={handleGenerateImage}
                          disabled={isGenerating || !aiPrompt.trim()}
                          className="p-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs disabled:opacity-50"
                        >
                          {isGenerating ? '...' : <MagicWandIcon className="w-4 h-4" />}
                        </button>
                      </div>
                    </div>
                  </div>
                  <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded text-xs dark:bg-gray-800 dark:text-white"
                        >
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full p-1.5 border border-gray-300 dark:border-gray-600 rounded text-xs dark:bg-gray-800 dark:text-white"
                        >
                          <option value="aspect-auto">Natural</option>
                          <option value="aspect-square">Square</option>
                          <option value="aspect-video">Video</option>
                        </select>
                      </div>
                    </div>
                    <div className="flex gap-4">
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span className="text-xs text-gray-700 dark:text-gray-300">Rounded</span>
                      </label>
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          ...
                          className="rounded text-blue-600 w-3 h-3"
                        />
                        <span className="text-xs text-gray-700 dark:text-gray-300">Shadow</span>
                      </label>
                    </div>
                  </div>
                  <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1">
                            <button
                              ...
                              className="p-1 text-purple-500 hover:text-purple-700 bg-purple-50 dark:bg-gray-700 hover:bg-purple-100 rounded"
                            >
                              <MagicWandIcon className="w-3.5 h-3.5" />
                            </button>
                            <button
                              onClick={() => removeImage(idx)}
                              className="p-1 text-red-500 hover:text-red-700 bg-red-50 dark:bg-gray-700 hover:bg-red-100 rounded"
                            >
          ...
          <div className="flex-1 bg-gray-100 dark:bg-gray-950 p-4 md:p-6 overflow-y-auto order-1 md:order-2 min-h-[30vh]">
            <div className="bg-white dark:bg-gray-800 rounded shadow-sm border border-gray-200 dark:border-gray-700 p-4 md:p-8 min-h-full">
              <div
                className={`grid grid-cols-1 sm:grid-cols-${Math.min(cols, 2)} md:grid-cols-${cols} ${gap}`}
              >
                {images.map((img, idx) => (
                  <div
                    key={idx}
                    className={`relative group overflow-hidden ${isRounded ? 'rounded-lg' : ''} ${editingIndex === idx ? 'ring-2 ring-purple-500 ring-offset-2' : ''}`}
                  >
                    <img
                      src={img}
                      alt={`Preview ${idx}`}
                      className={`w-full h-full object-cover ${isRounded ? 'rounded-lg' : ''} ${hasShadow ? 'shadow-md' : ''} ${aspectRatio}`}
                    />
                  </div>
                ))}
              </div>
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex justify-end gap-3 z-10">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 transition"
          >
            Cancel
          </button>
          <button
            ...
            className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm disabled:opacity-50 disabled:cursor-not-allowed transition font-medium"
          >
            Insert Layout
          </button>
        </div>
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-lg p-6 relative animate-fade-in-up">
      ...
      {selectedText && (
        <div className="mb-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-md border border-gray-200 dark:border-gray-600">
          <p className="text-sm text-gray-600 dark:text-gray-400 font-semibold">Selected Text:</p>
          <p className="text-sm text-gray-800 dark:text-gray-200 italic line-clamp-3">
            "{selectedText}"
          </p>
        </div>
      )}
        ...
        <div className="mb-4 pt-4 border-t border-gray-200 dark:border-gray-600">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={isLoading}
              className="flex-1 px-4 py-2 text-sm text-center bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Correct
            </button>
            <button
              onClick={() => onQuickAction('inflate')}
              disabled={isLoading}
              className="flex-1 px-4 py-2 text-sm text-center bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
            >
              Expand
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {suggestions.map((suggestion, index) => (
          <button
            ...
            className="px-3 py-1 text-sm bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full hover:bg-gray-300 dark:hover:bg-gray-600 transition"
          >
            {suggestion}
          </button>
        ))}
      ...
      <textarea
        ...
        placeholder="e.g., Turn this into a poem"
        className="w-full p-3 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
        rows={3}
        autoFocus
      />
      ...
      <div className="mt-6 flex justify-end space-x-3">
        <button
          onClick={onClose}
          className="px-4 py-2 rounded-md text-gray-700 bg-gray-200 hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500 transition"
        >
          Cancel
        </button>
        <button
          ...
          className="px-4 py-2 rounded-md text-white bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 disabled:cursor-not-allowed flex items-center transition"
        >
...
const CitationItem: React.FC<{ citation: GroundingChunk; index: number }> = ({
  citation,
  index,
}) => {
  const [copied, setCopied] = useState(false);
  const handleCopy = () => {
    navigator.clipboard.writeText(citation.web.uri);
    setCopied(true);
  ...
  return (
    <li className="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:shadow-md transition-shadow group">
      <div className="flex items-center overflow-hidden mr-3">
        <span className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 text-xs font-bold mr-3">
          {index + 1}
        </span>
      ...
      <button
        onClick={handleCopy}
        className="flex-shrink-0 p-2 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 bg-gray-50 dark:bg-gray-700 rounded-md hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors"
        title="Copy Link"
      >
        {copied ? (
...
const ToolbarButton: React.FC<{
  ...
  disabled?: boolean;
}> = ({ onClick, children, tooltip, buttonRef, disabled }) => (
  <button
    ref={buttonRef}
    onClick={onClick}
    onMouseDown={(e) => e.preventDefault()}
    disabled={disabled}
    className={`p-2 rounded relative group flex-shrink-0 transition-colors ${disabled ? 'text-gray-300 dark:text-gray-600 cursor-not-allowed' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700'}`}
    aria-label={tooltip}
  >
    {children}
    <span className="absolute bottom-full mb-2 w-max px-2 py-1 text-xs text-white bg-gray-900 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50">
      {tooltip}
    </span>
  </button>
);
...
const GeminiEditor: React.FC<GeminiEditorProps> = ({ value, onChange }) => {
  ...
  useEffect(() => {
    ...
    if (saved && saved !== valueRef.current) {
  ...
  }, [value, isSourceMode, isMarkdownMode]);
  useEffect(() => {
    if (isSourceMode) {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (dropdownPanelRef.current && !dropdownPanelRef.current.contains(event.target as Node)) {
        const isToolbarButtonClick =
          paragraphBtnRef.current?.contains(event.target as Node) ||
          fontBtnRef.current?.contains(event.target as Node) ||
          fontSizeBtnRef.current?.contains(event.target as Node) ||
          colorBtnRef.current?.contains(event.target as Node) ||
          tableBtnRef.current?.contains(event.target as Node);
        if (!isToolbarButtonClick) {
  ...
  const handleRestoreDraft = () => {
    if (draftContent) {
      ...
      addToHistory(draftContent);
      if (editorRef.current) editorRef.current.innerHTML = draftContent;
  ...
  const handleUndo = () => {
    if (historyIndex > 0) {
      ...
      onChange(content);
      if (editorRef.current && !isSourceMode && !isMarkdownMode)
        editorRef.current.innerHTML = content;
      if (isSourceMode) setSourceContent(content);
      if (isMarkdownMode) setMarkdownContent(turndownService.turndown(content));
    }
  };
  const handleRedo = () => {
  ...
  const handleDropdownToggle = (
    type: 'paragraph' | 'font' | 'fontSize' | 'color' | 'table',
    ...
    if (rect) {
      ...
      if (type === 'table') {
        setTableGridSize({ rows: 1, cols: 1 });
      }
    }
  };
  ...
  const applyFormat = (command: string, valueArg: string | null = null) => {
  ...
  const applyFontSize = (size: number) => {
    ...
    const applyStyleToNode = (node: Node): Node => {
      if (node.nodeType === Node.TEXT_NODE && node.nodeValue?.trim()) {
        ...
        const isBlock =
          ...
          ['LI', 'TABLE', 'TR', 'TH', 'TD'].includes(element.tagName);
        if (isBlock) {
          element.childNodes.forEach((child) => newElement.appendChild(applyStyleToNode(child)));
        } else {
          newElement.style.fontSize = `${size}px`;
          element.childNodes.forEach((child) => newElement.appendChild(child.cloneNode(true)));
        }
        return newElement;
      }
    ...
    try {
      ...
      Array.from(fragment.childNodes).forEach((child) => {
        styledFragment.appendChild(applyStyleToNode(child));
      });
  ...
  const applyInlineCode = () => {
    document.execCommand(
      'insertHTML',
      false,
      `<code class="bg-gray-200 dark:bg-gray-700 px-1 py-0.5 rounded text-sm font-mono">${window.getSelection()?.toString()}</code>`
    );
    if (editorRef.current) {
      const content = editorRef.current.innerHTML;
      onChange(content);
  ...
  const applyCodeBlock = () => {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      const codeBlock = `<pre class="bg-gray-900 text-white p-4 rounded-md overflow-x-auto"><code class="font-mono">${selectedText}</code></pre><p><br></p>`;
  ...
  const insertTable = (rows: number, cols: number) => {
    if (rows > 0 && cols > 0) {
      let table =
        '<table class="w-full border-collapse border border-gray-400 dark:border-gray-600"><tbody>';
      for (let i = 0; i < rows; i++) {
        table += '<tr>';
        for (let j = 0; j < cols; j++) {
          table += '<td class="border border-gray-300 dark:border-gray-700 p-2"><p><br></p></td>';
        }
        table += '</tr>';
      }
  ...
  const toggleSourceMode = () => {
    ...
    if (!isSourceMode) setSourceContent(value);
    setIsSourceMode(!isSourceMode);
  };
  const toggleMarkdownMode = () => {
    if (isSourceMode) setIsSourceMode(false);
    if (!isMarkdownMode) {
  ...
  const menuStructure: any = {
    Headings: [
      { label: 'Heading 1', onClick: () => applyFormat('formatBlock', '<h1>') },
      { label: 'Heading 2', onClick: () => applyFormat('formatBlock', '<h2>') },
      { label: 'Heading 3', onClick: () => applyFormat('formatBlock', '<h3>') },
      { label: 'Heading 4', onClick: () => applyFormat('formatBlock', '<h4>') },
      { label: 'Heading 5', onClick: () => applyFormat('formatBlock', '<h5>') },
      { label: 'Heading 6', onClick: () => applyFormat('formatBlock', '<h6>') },
    ],
    Inline: [
      { label: 'Bold', onClick: () => applyInlineStyle('bold') },
      { label: 'Italic', onClick: () => applyInlineStyle('italic') },
      { label: 'Underline', onClick: () => applyInlineStyle('underline') },
      { label: 'Strikethrough', onClick: () => applyInlineStyle('strikeThrough') },
      { label: 'Superscript', onClick: () => applyInlineStyle('superscript') },
      { label: 'Subscript', onClick: () => applyInlineStyle('subscript') },
      { label: 'Code', onClick: applyInlineCode },
    ],
    Align: [
      { label: 'Align Left', onClick: () => applyFormat('justifyLeft') },
      { label: 'Align Center', onClick: () => applyFormat('justifyCenter') },
      { label: 'Align Right', onClick: () => applyFormat('justifyRight') },
      { label: 'Align Justify', onClick: () => applyFormat('justifyFull') },
    ],
  };
  ...
  const handleAiRequest = async (promptOverride?: string) => {
    ...
    const contentToProcess = selectedText || editorRef.current?.innerText || '';
    try {
      const result: GeminiResponse = await enhanceTextWithGemini(contentToProcess, finalPrompt);
      const dirtyHtml = marked.parse(result.text) as string;
      ...
      if (isMarkdownMode || isSourceMode) {
        ...
        editorRef.current?.focus();
        let newContent = '';
        if (selectedText && editorRef.current && savedRange) {
          const selection = window.getSelection();
          if (selection) {
            selection.removeAllRanges();
            selection.addRange(savedRange);
          }
          const documentFragment = savedRange.createContextualFragment(cleanHtml);
          const lastNode = documentFragment.lastChild;
          savedRange.deleteContents();
          savedRange.insertNode(documentFragment);
          if (lastNode) {
      ...
      if (result.metadata && result.metadata.groundingChunks) {
        setCitations(result.metadata.groundingChunks);
  ...
  const handleQuickAction = (action: 'spellcheck' | 'inflate') => {
    let prompt = '';
    if (action === 'spellcheck') {
      prompt =
        'Correct spelling and grammar for the provided text. Only return the corrected text, maintaining the original meaning and tone as much as possible.';
    } else if (action === 'inflate') {
      prompt =
        'Elaborate and expand on the provided text. Add more details, examples, or explanations to make it more comprehensive. Use Google Search for up-to-date information if necessary.';
    }
    handleAiRequest(prompt);
  };
  const SubMenuItem: React.FC<{ item: { label: string; onClick: () => void } }> = ({ item }) => {
    let content;
    const commonClasses =
      'w-full text-left block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700';
    switch (item.label) {
      case 'Heading 1':
        content = <h1 className="text-3xl font-bold my-0">{item.label}</h1>;
        break;
      case 'Heading 2':
        content = <h2 className="text-2xl font-bold my-0">{item.label}</h2>;
        break;
      case 'Heading 3':
        content = <h3 className="text-xl font-bold my-0">{item.label}</h3>;
        break;
      case 'Heading 4':
        content = <h4 className="text-lg font-bold my-0">{item.label}</h4>;
        break;
      case 'Heading 5':
        content = <h5 className="text-base font-bold my-0">{item.label}</h5>;
        break;
      case 'Heading 6':
        content = <h6 className="text-sm font-bold my-0">{item.label}</h6>;
        break;
      case 'Bold':
        content = <span className="font-bold">{item.label}</span>;
        break;
      case 'Italic':
        content = <span className="italic">{item.label}</span>;
        break;
      case 'Underline':
        content = <span className="underline">{item.label}</span>;
        break;
      case 'Strikethrough':
        content = <span className="line-through">{item.label}</span>;
        break;
      case 'Superscript':
        content = (
          <span>
      ...
      case 'Subscript':
        ...
        break;
      case 'Code':
        content = <code className="font-mono">{item.label}</code>;
        break;
      default:
          ...
          <div className="flex items-center flex-nowrap gap-x-1">
            ...
            <div className="flex-shrink-0">
              <button
                ...
                onClick={() => handleDropdownToggle('paragraph', paragraphBtnRef)}
            ...
            <div className="flex-shrink-0">
              <button
                ...
                onMouseDown={(e) => e.preventDefault()}
                disabled={isMarkdownMode}
                onClick={() => handleDropdownToggle('font', fontBtnRef)}
                className={`flex items-center justify-between w-36 px-3 py-1.5 text-left bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md text-sm hover:bg-gray-100 dark:hover:bg-gray-700 ${isMarkdownMode ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
            ...
            <div className="flex-shrink-0">
              <button
                ...
                onMouseDown={(e) => e.preventDefault()}
                disabled={isMarkdownMode}
                onClick={() => handleDropdownToggle('fontSize', fontSizeBtnRef)}
                className={`flex items-center justify-between w-32 px-3 py-1.5 text-left bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md text-sm hover:bg-gray-100 dark:hover:bg-gray-700 ${isMarkdownMode ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
                <span>Font Size</span>
                <ChevronDownIcon className="w-4 h-4" />
            ...
            {/* Insert Buttons */}
            <div className="flex items-center flex-shrink-0">
              <ToolbarButton onClick={applyLink} tooltip="Insert Link" disabled={isMarkdownMode}>
            ...
            <div className="flex items-center flex-shrink-0">
              <ToolbarButton
                ...
                tooltip={isMarkdownMode ? 'Rich Text View' : 'Markdown Mode'}
              >
                <MarkdownIcon
                  className={`w-5 h-5 ${isMarkdownMode ? 'text-blue-600 dark:text-blue-400' : ''}`}
                />
              </ToolbarButton>
              <ToolbarButton onClick={toggleSourceMode} tooltip="Source Code">
                <SourceIcon
                  className={`w-5 h-5 ${isSourceMode ? 'text-blue-600 dark:text-blue-400' : ''}`}
                />
              </ToolbarButton>
              <ToolbarButton
        ...
        >
          {activeDropdown === 'paragraph' && (
            <div
              ...
              className="w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg"
            >
              {mainCategories.map((category) => (
                <div
                  key={category}
                  onMouseEnter={() => setActiveSubMenu(category)}
                  className="relative flex justify-between items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer"
                >
                  <span>{category}</span>
              ...
              {activeSubMenu && menuStructure[activeSubMenu] && (
                <div className="absolute top-0 left-full ml-1 w-56 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md shadow-lg">
                  {menuStructure[activeSubMenu].map((item) => (
                ...
                {Array.from({ length: 100 }).map((_, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`w-5 h-5 border border-gray-300 dark:border-gray-600 cursor-pointer transition-colors ${
                        isHighlighted
