...
export const Dialog = ({ isOpen, onClose, children, title, description, size = 'md', zIndex = 60 }: DialogProps) => {
  ...
  return createPortal(
    <div className="fixed inset-0 flex items-end sm:items-center justify-center sm:p-4" style={{ zIndex }}>
      <div 
        ref={overlayRef}
      ...
      <div className={`relative z-50 w-full ${sizeClasses[size]} max-h-[95dvh] sm:max-h-[85vh] flex flex-col gap-4 border-t sm:border border-border bg-background p-6 shadow-lg animate-in slide-in-from-bottom sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300 rounded-t-xl sm:rounded-lg`}>
        <div className="flex flex-col space-y-1.5 text-left mb-2 shrink-0 pr-8">
          {title && <h2 className="text-lg font-semibold leading-none tracking-tight">{title}</h2>}
          {description && <p className="text-sm text-muted-foreground">{description}</p>}
        </div>
        
        <div className="overflow-y-auto -mx-6 px-6 flex-1 custom-scrollbar">
            {children}
        </div>
