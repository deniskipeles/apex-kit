                            ...
                            <div className="bg-secondary/10 rounded-md border border-border overflow-hidden max-h-[300px] overflow-y-auto">
                                {backups.length === 0 ? (
                                    ...
                                    <div className="divide-y divide-border">
                                        {backups.map((b) => (
                                            <div key={b.name} className="flex items-center justify-between p-3 hover:bg-secondary/20 transition-colors">
                                                <div className="flex items-center gap-3">
                                                    <FileArchive className="h-5 w-5 text-primary/70" />
                                                    <div>
                                                        <div className="text-sm font-medium">{b.name}</div>
                                                        <div className="text-xs text-muted-foreground">{new Date(b.created).toLocaleString()} • {formatFileSize(b.size)}</div>
                                                    </div>
                                                </div>
                                                <div className="flex gap-1">
                                                    <Button size="icon" variant="ghost" onClick={() => handleDownload(b.name)} title="Download"><Download className="h-4 w-4" /></Button>
                                                    <Button size="icon" variant="ghost" onClick={() => handleRestore(b.name)} title="Restore" className="text-amber-500 hover:text-amber-600 hover:bg-amber-500/10">
                                                        {isRestoring && restoreTarget === b.name ? <Loader2 className="h-4 w-4 animate-spin" /> : <RotateCcw className="h-4 w-4" />}
                                                    </Button>
                                                </div>
                                            </div>
                                        ))}
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex flex-col md:flex-row gap-3 pt-4 border-t items-end">
                        <div className="flex-1 w-full space-y-1">
                            ...
                            <Input placeholder="e.g. Daily Clean" value={newCronName} onChange={(e: any) => setNewCronName(e.target.value)} className="h-9" />
                        </div>
                        <div className="flex-[2] w-full space-y-1">
                            <Label className="text-xs">Payload (Script / Webhook)</Label>
                            <Input placeholder="script-name or /api/..." value={newCronCmd} onChange={(e: any) => setNewCronCmd(e.target.value)} className="h-9 font-mono" />
                        </div>
