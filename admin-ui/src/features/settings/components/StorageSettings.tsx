...
import { HardDrive, Cloud, Server, ArrowRightLeft } from 'lucide-react';
...
export const StorageSettings = ({ settings, onChange }: StorageSettingsProps) => {
    ...
    const handleMigration = async () => {
        ...
        try {
            ...
            if (res.errors > 0) {
                toast(`Migration completed with issues. Processed: ${res.processed}, Errors: ${res.errors}`, 'warning');
            } else {
                ...
                <CardContent className="space-y-4">
                    ...
                    <div className="flex justify-end pt-2">
                        <Button
                            ...
                            isLoading={isTesting}
                            disabled={!settings.storage.s3.bucket} // Disable if bucket empty
                        >
                ...
                <CardContent>
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                        <div className="text-sm text-muted-foreground">
                            ...
                            Use this after changing your storage driver.
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-background border border-border rounded-lg shadow-xl p-6 max-w-sm w-full animate-in zoom-in-95">
                        ...
                        <p className="text-sm text-muted-foreground mb-4">
                            This will copy all files from <strong>{migrationDirection === 'local_to_s3' ? 'Local Disk' : 'S3 Bucket'}</strong> to <strong>{migrationDirection === 'local_to_s3' ? 'S3 Bucket' : 'Local Disk'}</strong>.
                            <br /><br />
                            This process runs in the background and may take time depending on file size.
                        </p>
                        <div className="flex justify-end gap-2">
                            <Button variant="ghost" onClick={() => setShowMigrationModal(false)}>Cancel</Button>
