...
export const StorageSettings = ({ settings, onChange, onSave }: StorageSettingsProps) => {
  ...
  const [migrationDirection, setMigrationDirection] = useState<'local_to_s3' | 's3_to_local'>(
    'local_to_s3'
  );
  ...
  const updateS3 = (updates: any) => {
    onChange({ storage: { ...settings.storage, s3: { ...settings.storage.s3, ...updates } } });
  };
  ...
  const handleMigration = async () => {
    ...
    try {
      ...
      if (res.errors > 0) {
        toast(
          `Migration completed with issues. Processed: ${res.processed}, Errors: ${res.errors}`,
          'warning'
        );
      } else {
        toast(`Success! Migrated ${res.processed} files.`, 'success');
      }
    } catch (e: any) {
      console.error(e);
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            <HardDrive className="h-4 w-4" /> Active Storage Driver
          </CardTitle>
        </CardHeader>
      ...
      <Card
        className={`transition-opacity duration-300 ${settings.storage.activeDriver === 's3' ? 'opacity-100' : 'opacity-50 pointer-events-none grayscale'}`}
      >
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex justify-end pt-2">
            <Button
              ...
              isLoading={isTesting}
              disabled={!settings.storage.s3.bucket}
            >
      ...
      <Card className="border-dashed border-border bg-secondary/5">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-base">
            <ArrowRightLeft className="h-4 w-4" /> Data Migration
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="text-sm text-muted-foreground">
              Move existing files between Local Storage and S3.
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-background border border-border rounded-lg shadow-xl p-6 max-w-sm w-full animate-in zoom-in-95">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              This will copy all files between storages. Process runs in the background.
            </p>
            <div className="flex justify-end gap-2">
              <Button variant="ghost" onClick={() => setShowMigrationModal(false)}>
                Cancel
              </Button>
