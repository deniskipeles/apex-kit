  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (s: Script) => (
        <Badge
          variant="outline"
          className={`uppercase text-[10px] ${s.trigger_type === 'manual' ? 'border-blue-500/30 text-blue-500' : 'border-orange-500/30 text-orange-500'}`}
        >
    ...
    <div className="space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Custom server-side logic, endpoints, and event hooks.
          </p>
        </div>
        <div className="flex gap-2">
          <ImportExportToolbar
            onExport={(format) => scriptsService.export(format)}
      ...
      {!isRoot && meta.transparency_enabled && (
        <Alert className="bg-blue-500/10 text-blue-500 border-blue-500/20">
          <strong>Transparency Mode:</strong> Your tenancy provider is running {meta.root_total}{' '}
          root-level scripts. ({sharedScripts.filter((s) => s.visibility === 'public').length}{' '}
          Public APIs,{' '}
          {meta.root_total - sharedScripts.filter((s) => s.visibility === 'public').length} Private
          Background Tasks).
        </Alert>
      )}
      ...
      {!isRoot && !meta.transparency_enabled && sharedScripts.length > 0 && (
        <Alert className="bg-secondary/50 text-muted-foreground border-border">
          Your provider has exposed {sharedScripts.length} public root scripts for you to invoke via{' '}
          <code>$run.script()</code> OUT of {meta.root_total}.
        </Alert>
      )}
