...
import { Search, BrainCircuit, Zap, RefreshCw, ArrowRight, Image as ImageIcon, Type, X, UploadCloud } from 'lucide-react';
            ...
            <div className="flex flex-col md:flex-row gap-6">
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 animate-in fade-in zoom-in-95 duration-200">
                                        ...
                                        {imageBase64 ? (
                                            <div className="relative rounded-lg overflow-hidden border border-border group bg-black/50 flex items-center justify-center h-40">
                                                <img src={imageBase64} alt="Query" className="max-h-full max-w-full object-contain" />
                                                <button 
                                        ...
                                        <p className="text-[10px] text-muted-foreground text-center">Requires a visual embedding model (e.g. CLIP).</p>
                                    </div>
                                )}
                                ...
                                <Button 
                                    ...
                                    disabled={!selectedColId || (searchType === 'text' && !query) || (searchType === 'image' && !imageBase64)}
                                >
                ...
                <div className="flex-1">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        {results.length > 0 && <span className="text-xs text-muted-foreground">Top {results.length} matches by cosine similarity</span>}
                    </div>
                    ...
                    <div className="space-y-3">
                        {isSearching ? (
                            ...
                            results.map((record, i) => {
                                ...
                                const imageFieldKey = Object.keys(record.data).find(k => 
                                    typeof record.data[k] === 'string' && 
                                    record.data[k].match(/\.(jpg|jpeg|png|webp|gif)$/i)
                                );
                                const imageUrl = imageFieldKey ? apiClient.files.getFileUrl(record.data[imageFieldKey]) : null;

                                return (
                                    <div key={record.id} className="bg-card border border-border rounded-lg p-4 transition-all hover:border-primary/50 hover:shadow-md group flex gap-4">
                                        ...
                                        {imageUrl && (
                                            <div className="h-16 w-16 shrink-0 rounded-md overflow-hidden bg-black/10 border border-border">
                                                <img src={`${imageUrl}?thumb=100x100`} alt="Thumbnail" className="h-full w-full object-cover" />
                                            </div>
                                        )}
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-start mb-2">
                                                ...
                                                <Badge variant="outline" className={`font-mono text-[10px] shrink-0 ${i === 0 ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : ''}`}>
                                                    Match #{i+1}
                                                </Badge>
                                            </div>
                                            ...
                                            <div className="text-sm text-muted-foreground line-clamp-2 mb-3">
                                                {Object.entries(record.data)
                                                    .filter(([k, v]) => typeof v === 'string' && v.length > 20 && k !== imageFieldKey)
                                                    .map(([k, v]) => v)
