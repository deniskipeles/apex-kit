...
import { Send, Bot, User, Code, ExternalLink, RefreshCw, UploadCloud, X, History, Box, Calendar, FileJson } from 'lucide-react';
...
export const AiSessionPanel = ({ session, onClose, onUpdate }: AiSessionPanelProps) => {
    ...
    const loadVersions = async () => {
        ...
        try {
            ...
            const relevant = appName
                ? allPlugins.filter(p => p.name === appName)
                : [];
            ...
            actions={
                <div className="flex gap-2 w-full">
                    ...
                    <Button
                        ref={manifestBtnRef}
                        variant="outline"
                        onClick={() => { setShowManifest(!showManifest); setShowVersions(false); }}
                        className="flex-1"
            ...
            <div className="flex flex-col h-[calc(100vh-200px)]">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
                    ...
                    {session.messages.map((msg, idx) => (
                        <div key={idx} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground'}`}>
                                {msg.role === 'user' ? <User className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
                            </div>
                            <div className={`rounded-lg p-3 max-w-[85%] text-sm whitespace-pre-wrap ${msg.role === 'user' ? 'bg-primary/10 border border-primary/20' : 'bg-card border border-border'}`}>
                                {msg.content}
                            </div>
                        </div>
                    ))}
                ...
                <div className="p-4 border-t border-border mt-auto bg-background/50 backdrop-blur-sm">
                    ...
                    <div className="flex justify-between items-center px-1">
                        <span className="text-[10px] font-bold uppercase text-muted-foreground tracking-wider">AI Model</span>
                        <div className="w-48">
                            <Select
                                value={model}
                ...
                >
                    <div className="flex items-center justify-between p-3 border-b border-border bg-secondary/10">
                        ...
                        <button onClick={() => setShowManifest(false)} className="hover:bg-secondary rounded p-1"><X className="h-4 w-4" /></button>
                    </div>
                    <div className="flex-1 overflow-auto p-0 bg-[#1e1e1e]">
                        <pre className="text-[10px] font-mono text-blue-100 p-4 leading-relaxed">
                            {session.current_manifest ? JSON.stringify(session.current_manifest, null, 2) : "// No manifest"}
                        </pre>
                    </div>
                </Overlay>
                ...
                >
                    <div className="space-y-4">
                        <p className="text-sm text-muted-foreground">Select a version to view its code manifest.</p>
                        <div className="border rounded-md divide-y divide-border max-h-[400px] overflow-y-auto">
                            {isLoadingVersions ? (
                                <div className="p-8 text-center text-xs text-muted-foreground"><RefreshCw className="h-4 w-4 animate-spin inline mr-2" /> Loading history...</div>
                            ) : versions.length === 0 ? (
                                <div className="p-8 text-center text-xs text-muted-foreground">No published versions found.</div>
                            ) : (
                                versions.map(v => (
                                    <button
                                        key={v.id}
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            {v.description && (
                                                <p className="text-[11px] text-muted-foreground line-clamp-1">{v.description}</p>
                                            )}
                                        </div>
                                    </button>
                                ))
