...
import { 
    ...
    Maximize2, X, Send, ChevronDown, LayoutTemplate,
    Terminal, Zap, ExternalLink, Paperclip, FileCode, FileJson, Check, AlertTriangle
} from 'lucide-react';
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center gap-4 w-full max-w-3xl pointer-events-none">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-transparent to-black/5">
                        {session?.messages.map((msg, i) => (
                            <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                <div className={`max-w-[85%] rounded-lg p-3 text-sm whitespace-pre-wrap shadow-sm ${msg.role === 'user' ? 'bg-primary text-primary-foreground' : 'bg-card border border-border text-foreground'} ${msg.role === 'error' ? 'bg-destructive/10 text-destructive border-destructive/20' : ''}`}>
                                    {msg.content}
                                </div>
                            </div>
                        ))}
                        ...
                        {session?.diff_summary && (
                             <div className="mx-4 my-2 border border-amber-500/30 bg-amber-500/5 rounded-lg overflow-hidden animate-in fade-in slide-in-from-bottom-2">
                                ...
                                <div className="p-3">
                                    <pre className="text-xs font-mono text-foreground/80 whitespace-pre-wrap">{session.diff_summary}</pre>
                                </div>
                                <div className="p-2 bg-background/50 flex gap-2 justify-end border-t border-amber-500/10">
                                    <Button size="sm" variant="outline" onClick={() => { /* Logic to discard? */ }} className="h-7 text-xs">
                                        Refine
                                    </Button>
                                    <Button size="sm" onClick={handleApply} isLoading={isApplying} className="h-7 text-xs bg-amber-500 hover:bg-amber-600 text-black">
                    ...
                    <div className="p-3 border-t border-border bg-background">
                        <div className="relative flex items-center gap-2">
                            ...
                            <Input 
                                ...
                                placeholder={session?.diff_summary ? "Refine these changes or describe new ones..." : "Describe a feature, schema change, or UI tweak..."}
                            ...
                            >
                                <div className="w-64 bg-popover border border-border rounded-lg shadow-xl overflow-hidden flex flex-col max-h-[300px]">
                                    ...
                                    <div className="overflow-y-auto flex-1 p-1">
                                        {/* Collections */}
                                        <div className="px-2 py-1 text-[10px] text-muted-foreground font-semibold">Collections</div>
                                        {collections.map(c => (
                                            <button key={c.id} onClick={() => handleAttach('collection', c.id)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-accent rounded flex items-center gap-2">
                                                <Database className="h-3 w-3 text-blue-500" /> {c.name}
                                            </button>
                                        ))}
                                        
                                        {/* Templates */}
                                        <div className="px-2 py-1 text-[10px] text-muted-foreground font-semibold mt-1">Templates</div>
                                        {templates.map(t => (
                                            <button key={t.id} onClick={() => handleAttach('template', t.id)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-accent rounded flex items-center gap-2">
                                                <LayoutTemplate className="h-3 w-3 text-purple-500" /> {t.slug}
                                            </button>
                                        ))}

                                        {/* Scripts */}
                                        <div className="px-2 py-1 text-[10px] text-muted-foreground font-semibold mt-1">Scripts</div>
                                        {scripts.map(s => (
                                            <button key={s.id} onClick={() => handleAttach('script', s.id)} className="w-full text-left px-2 py-1.5 text-xs hover:bg-accent rounded flex items-center gap-2">
                ...
                <div className="w-full h-[600px] bg-background border border-border rounded-xl shadow-2xl flex flex-col pointer-events-auto overflow-hidden animate-in slide-in-from-bottom-5">
                    <div className="p-2 border-b border-border flex gap-2 items-center bg-secondary/20">
                        ...
                        <div className="flex gap-1 border-l border-border/50 pl-2">
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => refreshResources()} title="Refresh Templates">
                                <RefreshCw className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => window.open(previewUrl || '', '_blank')} disabled={!previewUrl} title="Open in New Tab">
                                <ExternalLink className="h-3.5 w-3.5" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setActiveTab(null)}>
                                <X className="h-4 w-4" />
                            </Button>
                        </div>
                    </div>
            ...
            {activeTab === 'tools' && (
                <div className="bg-popover border border-border rounded-xl shadow-xl p-2 mb-2 grid grid-cols-2 gap-2 w-64 pointer-events-auto animate-in zoom-in-95 slide-in-from-bottom-2">
                    <Button variant="ghost" className="justify-start gap-2 h-9 text-xs" onClick={() => window.open(`${APP_CONFIG.apiBaseUrl}/sandbox/${sessionId}/scalar`, '_blank')}>
                        <Terminal className="h-3.5 w-3.5 text-blue-500" /> API Docs
                    </Button>
                    <Button variant="ghost" className="justify-start gap-2 h-9 text-xs" onClick={() => {/* Trigger re-index */}}>
            ...
            <div className="flex items-center gap-2 p-1.5 bg-foreground/90 backdrop-blur-md text-background rounded-full shadow-2xl border border-white/10 pointer-events-auto hover:scale-[1.01] transition-transform duration-200">
                ...
                <div className="flex items-center gap-1">
                    <ToolbarButton icon={<Sparkles className="h-4 w-4" />} label="Architect" active={activeTab === 'chat'} onClick={() => toggleTab('chat')} />
                    <ToolbarButton icon={<Play className="h-4 w-4" />} label="Preview" active={activeTab === 'preview'} onClick={() => toggleTab('preview')} />
                    <ToolbarButton icon={<LayoutTemplate className="h-4 w-4" />} label="Tools" active={activeTab === 'tools'} onClick={() => toggleTab('tools')} />
                </div>
                <div className="pl-2 pr-1 border-l border-white/20">
                    <button onClick={() => setIsOpen(false)} className="p-1.5 rounded-full hover:bg-white/20 text-white/70 hover:text-white transition-colors">
            ...
            {!isOpen && (
                <Button className="absolute bottom-4 right-4 rounded-full h-12 w-12 shadow-xl pointer-events-auto" onClick={() => setIsOpen(true)}>
