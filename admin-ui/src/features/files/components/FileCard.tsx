...
export const FileCard: React.FC<FileCardProps> = ({ file, onClick, onDelete }) => {
  return (
    <div
      ...
      onClick={onClick}
    >
      <FileThumbnail url={file.url + `?thumb=` + APEX_FILES_THUMB_SIZE} mimeType={file.mimeType} name={file.name} className="aspect-[4/3] bg-secondary/20" />
      ...
      <div className="p-3">
        <div className="flex justify-between items-start gap-2">
          ...
          <Button size="icon" variant="ghost" className="h-6 w-6 -mr-1 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => { e.stopPropagation(); onDelete(e); }}>
