  ...
  return (
    <div className="md:hidden w-full relative">
      <button
        ...
        className="flex w-full items-center justify-between gap-2 rounded-md border border-border bg-background px-3 py-2 text-sm font-bold shadow-sm"
      >
        <span className="truncate flex items-center gap-2">
          <Database className="h-4 w-4 text-primary" /> {active || 'Select Collection'}
        </span>
        ...
        <div className="flex flex-col rounded-md border border-border bg-popover p-1 shadow-xl max-h-[300px] overflow-y-auto">
          {collections.map((c) => (
            <button
              ...
              className={`w-full rounded-sm px-3 py-2.5 text-left text-sm truncate flex items-center gap-2 ${active === c.name ? 'bg-accent font-semibold text-primary' : 'hover:bg-accent'}`}
            >
              {c.name}
            </button>
          ))}
  ...
  return (
    <>
      ...
      <Overlay
        ...
        align="end"
        width={200}
        className="bg-popover border border-border shadow-xl rounded-md p-1 z-50"
      >
        <div className="flex flex-col text-sm">
          ...
          <button
            ...
            className="flex items-center gap-2 px-2 py-2 hover:bg-accent rounded-sm text-left"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-2 py-2 hover:bg-accent rounded-sm text-left"
          >
            <Fingerprint className="h-3.5 w-3.5" /> Re-Index Search
          </button>
          <button
            onClick={() => {
              onRevectorize();
            ...
            className="flex items-center gap-2 px-2 py-2 hover:bg-accent rounded-sm text-left"
          >
            <Zap className="h-3.5 w-3.5" /> Re-Vectorize AI
          </button>
        </div>
      </Overlay>
    </>
...
export const RecordsListPage = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!searchText) {
        ...
        const res = await recordsService.list(
          targetCol.id,
          page,
          perPage,
          expandStr,
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${selectedRecordIds.length} records? This action cannot be undone.`
      )
    )
      return;
  ...
  const columns = [
    {
      ...
      renderCell: (r: AppRecord) => (
        <span
          className="font-mono text-[10px] text-muted-foreground bg-secondary/50 px-1.5 py-0.5 rounded truncate border border-transparent hover:border-primary/20"
          title={r.id}
        >
          #{r.id}
        </span>
        ...
        switch (f.type) {
          case 'bool':
            return val ? (
              ...
              <Badge variant="secondary" className="h-5 px-1.5 text-[10px] gap-1 opacity-70">
                <XIcon className="h-3 w-3" /> False
              </Badge>
            );
          case 'date':
            return (
          ...
          case 'owner': {
            ...
            return (
              <Badge
                variant="outline"
                className="font-mono text-[10px] h-5 border-primary/20 text-primary bg-primary/5 truncate max-w-[120px]"
                title={`ID: ${val}`}
              >
                {label}
              </Badge>
          ...
          case 'json':
            return (
              <code className="text-[10px] font-mono bg-muted px-1.5 py-0.5 rounded text-muted-foreground border border-border truncate max-w-[120px] block">
                {JSON.stringify(val).substring(0, APEX_TRUNCATION_SIZE)}...
              </code>
            );
          case 'url':
            return (
              <a
                ...
                className="text-primary hover:underline text-xs flex items-center gap-1 truncate max-w-[150px]"
              >
                <LinkIcon className="h-3 w-3 flex-shrink-0" />{' '}
                <span className="truncate">{String(val).replace(/(^\w+:|^)\/\//, '')}</span>
              </a>
            );
            ...
            return (
              <div className="flex items-center gap-2 group">
                <div className="h-8 w-8 rounded overflow-hidden bg-secondary border border-border shrink-0">
                  <FileThumbnail
                    url={url + '?thumb=100x100'}
                    mimeType={guessMimeType(String(val))}
                  />
                </div>
          ...
          case 'text':
            return (
              <span
                ...
                title={String(apiClient.stripHtmlTags(val))}
              >
                {String(apiClient.stripHtmlTags(val))}
              </span>
            );
          default:
            return (
              <span
                className="text-sm truncate block max-w-[200px] text-foreground/90"
                title={String(val)}
              >
                {String(val)}
              </span>
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] w-full md:flex-row bg-background/50">
      ...
      <div className="w-60 border-r bg-background/50 backdrop-blur supports-[backdrop-filter]:bg-background/60 hidden md:flex flex-col flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1 custom-scrollbar">
          {cols.map((c) => (
            ...
            >
              ...
              {activeCol === c.name && (
                <div className="h-1.5 w-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(var(--primary))]"></div>
              )}
            </button>
          ))}
        </div>
        <div className="p-3 border-t">
          <Button
            ...
            onClick={() => {
              setActiveCol('');
              setViewMode('create'); /* Or handle collection creation explicitly */
            }}
          >
            ...
            <div className="hidden md:block">
              <div className="flex items-center gap-3">
                ...
                {totalItems > 0 && (
                  <Badge
                    variant="secondary"
                    className="text-[10px] h-5 px-1.5 font-mono flex-shrink-0"
                  >
                    {totalItems} total
                  </Badge>
                )}
          ...
          <div className="flex flex-wrap items-center gap-2 self-start sm:self-end lg:self-auto w-full lg:w-auto justify-end">
            ...
            {selectedRecordIds.length > 0 && (
              <Button
                variant="destructive"
                size="sm"
                className="h-8 text-xs shadow-sm whitespace-nowrap"
      ...
      <RecordPreviewPanel
        ...
        onDelete={() => {
          if (previewRec)
            recordsService.delete(previewRec.collectionId, previewRec.id).then(() => {
              loadData();
              setPreviewRec(null);
            });
        }}
      ...
      <Dialog
        isOpen={isImportOpen}
        onClose={() => !isImporting && setIsImportOpen(false)}
        title="Import Data"
        size="sm"
      >
        <form onSubmit={handleImport} className="space-y-4">
          <div className="text-sm text-muted-foreground">
            Upload a <b>JSON</b> or <b>CSV</b> file. Schema will be inferred if the collection is
            empty.
          </div>
