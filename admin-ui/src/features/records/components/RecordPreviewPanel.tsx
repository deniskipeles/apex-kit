...
const getExpandedLabel = (item: any): string => {
   ...
   const explicit = data.email || data.name || data.title || data.slug || data.username || data.label;
  ...
  const renderFieldContent = (field: SchemaField) => {
    ...
    switch (field.type) {
      case 'file': {
        ...
        if (!filename || filename === 'null') return <span className="text-muted-foreground italic">No file</span>;
      ...
      case 'owner': {
            ...
            return (
                <div className="flex flex-wrap gap-2">
                    {expanded.map((item: any) => (
                        <Badge key={item.id} variant="outline" className="bg-background hover:bg-secondary/50 transition-colors py-1 pl-1 pr-2 gap-1.5 border-primary/20 text-foreground cursor-default">
                            <div className="w-1.5 h-1.5 rounded-full bg-primary/50"></div>
                            <span className="font-medium truncate max-w-[150px]">{getExpandedLabel(item)}</span>
                        </Badge>
                    ))}
                </div>
            );
        ...
        if (expanded && !Array.isArray(expanded)) {
            return (
                <Badge variant="outline" className="bg-background py-1 pl-1 pr-2 gap-1.5 border-primary/20 text-foreground">
                    <div className="w-1.5 h-1.5 rounded-full bg-primary/50"></div>
        ...
        return <span className="font-mono text-xs text-muted-foreground">{String(rawId || '-')}</span>;
      }
      ...
      case 'bool':
        return value ? (
            <Badge variant="success" className="text-[10px] px-2">True</Badge>
        ) : (
            <Badge variant="secondary" className="text-[10px] px-2 opacity-70">False</Badge>
        );
      ...
      default:
        ...
        return typeof value === 'object' 
            ? <pre className="text-[10px] whitespace-pre-wrap font-mono">{JSON.stringify(value, null, 2)}</pre> 
      ...
      <div className="relative w-full max-w-lg h-full bg-background border-l border-border shadow-2xl animate-in slide-in-from-right flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
           <div className="grid grid-cols-2 gap-4 p-4 bg-secondary/10 rounded-lg border border-border">
              <div><span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Created</span><p className="text-xs font-mono mt-1">{new Date(record.created).toLocaleString()}</p></div>
              <div><span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Updated</span><p className="text-xs font-mono mt-1">{new Date(record.updated).toLocaleString()}</p></div>
              <div className="col-span-2 pt-2 border-t border-border/50"><span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">System ID</span><p className="text-xs font-mono mt-1 select-all break-all text-muted-foreground">{record.id}</p></div>
           </div>
              ...
              {collection.schema.map(field => (
                 <div key={field.name} className="group">
                    <div className="flex items-center gap-2 mb-2">
                        <span className="text-sm font-semibold capitalize text-foreground">{field.name}</span>
                        <Badge variant="secondary" className="text-[9px] h-4 px-1.5 uppercase font-mono text-muted-foreground">{field.type}</Badge>
                    </div>
        ...
        <div className="p-4 border-t flex gap-3 bg-background">
           <Button className="flex-1" variant="outline" onClick={onEdit}><Edit className="mr-2 h-4 w-4" /> Edit Record</Button>
