# TinyBase Comprehensive Developer Documentation
...
TinyBase is a monolithic, single-binary Backend-as-a-Service. Unlike traditional frameworks, it combines the database, API server, and logic engine into one process.
...
TinyBase uses a strict schema definition that validates data *before* it hits the JSON storage.
...
TinyBase allows complex filtering and relational expansion in a single HTTP request.
...
**How it works:**
1.  TinyBase parses the expansion tree.
...
TinyBase includes a custom JavaScript runtime. Scripts run in a sandboxed environment on the server.
...
TinyBase can render HTML on the server, acting as a web server, not just an API.
...
TinyBase allows defining "AI Actions" which are prompt templates exposed as API endpoints.
...
TinyBase broadcasts database change events via WebSocket.
