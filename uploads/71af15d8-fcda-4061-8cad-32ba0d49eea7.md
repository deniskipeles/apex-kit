# TinyBase + Microframe: Progressive Enhancement Guide

This guide explains how to use **Microframe** within **TinyBase Templates** to add rich interactivity to your server-rendered pages.

## The Philosophy: Progressive Enhancement

TinyBase (server-side) handles the heavy lifting: fetching data, security, and rendering the initial HTML via Tera. Microframe (client-side) takes over specific "islands" of the page to handle complex state, animations, or immediate user feedback without a server round-trip.

---

## 1. Setup

First, ensure `microframe.js` is accessible to your frontend. Since TinyBase serves static files from the `static/` directory, place the built `microframe.js` there.
...
<script type="module">
    ...
    class LikeButton extends Component {
        ...
        async toggleLike() {
            ...
            try {
                // Use TinyBase Script Runner Endpoint
...
TinyBase works beautifully with HTMX. Microframe fills the gaps where HTMX feels "clunky" (like complex local state or high-frequency updates).
...
*   **Global (Tailwind):** TinyBase uses Tailwind. If you want your Microframe component to use Tailwind classes, you must inject the Tailwind stylesheet into the Shadow DOM, OR avoid Shadow DOM (Microframe supports Shadow DOM by default, but you can modify it to use Light DOM if you prefer global styles).
...
*   **TinyBase Scripts (in DB, executed via `/run/`):** Run on the **Server**. Secure. Can access `$db`.
