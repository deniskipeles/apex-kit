...
# TinyBase GraphQL API Documentation

**Version:** 2.4
**Endpoint:** `/graphql`
**Playground:** Open `/graphql` in your browser to access the interactive GraphiQL playground.

TinyBase provides a dynamic, **Read-Only GraphQL API**. The schema is automatically generated based on your Collections and their Relationships defined in the Admin Dashboard.
...
To filter data, use the `where` argument. This argument accepts a **JSON Scalar**. The syntax follows the **TinyBase Filters API** (MongoDB-style).
...
The power of GraphQL lies in fetching related data. TinyBase automatically maps:
...
TinyBase Schema types map to GraphQL types as follows:

| TinyBase Type | GraphQL Type | Notes |
| :--- | :--- | :--- |
