...
# TinyBase Real-Time API (WebSocket)

**Version:** 2.4
**Endpoint:** `ws://localhost:5000/ws`

TinyBase provides a high-performance, persistent WebSocket connection for listening to database changes in real-time. Unlike polling, this pushes data to your client immediately when a record is **Inserted**, **Updated**, or **Deleted**.
...
ws.onopen = () => {
    console.log("Connected to TinyBase Real-time Stream");
};
```
...
The `filter` payload accepts the **TinyBase JSON Filter Syntax** (MongoDB-style). This filtering happens **in-memory** on the server, ensuring extremely low latency.
...
Here is a robust class for handling TinyBase subscriptions with auto-reconnection.

```javascript
class TinyBaseSubscriber {
    constructor(url) {
        this.url = url;
        this.socket = null;
        this.listeners = [];
    ...
    connect() {
        ...
        this.socket.onopen = () => {
            console.log("TinyBase Connected");
...
const stream = new TinyBaseSubscriber("ws://localhost:5000/ws");
